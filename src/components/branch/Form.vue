<template lang="html">
  <v-card>
    <v-card-title>
      <span class="headline">{{ title }} Branch</span>
    </v-card-title>
    <v-card-text>
      <v-form ref="form" v-model="valid">
        ...
        ></v-text-field>
        <v-btn ref="submit" id="frm_branch_submit" @click="add">{{ title }}</v-btn>
...
export default {
  name: 'branch-form',
  props: ['title', 'branch', 'formErrors', 'disabledFields', 'autofocus'],
  data() {
    return {
      valid: true,
      codeRules: [(code) => {
