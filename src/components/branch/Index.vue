<template>
    <div>
      <master-data-table-toolbar title="Branches" @search="setSearch" @create="openDialog('create')" @refresh="getItems" />
      <v-data-table
        :headers="headers"
        :items="items"
      ...
      <v-dialog v-model="dialog" max-width="300px" lazy persistent>
        <create-branch-form v-if="dialogComponent === 'create'" @create="postItemCreate" @cancel="close" />
        <edit-branch-form v-else-if="dialogComponent === 'edit'" :id="editedId" @update="postItemUpdate" @cancel="close" />
      </v-dialog>
...
import CreateBranchForm from '@/components/branch/Create';
import EditBranchForm from '@/components/branch/Edit';
...
export default {
  ...
  methods: {
    ...
    getItems() {
      ...
      return fetchMasterList('branches', this.search, page, rowsPerPage, sortBy, descending ).then(response => {
    ...
    onItemDelete(item) {
      ...
      this.loading = true;            
      deleteMasterItem('branches', id).then(() => {
        const snackbar = {
          text: `Branch "${id}" deleted`,
          show: true,
          color: 'success'
        };
        ...
        const snackbar = {
          text: `Branch "${id}" could not be deleted`,
          show: true,
          color: 'error'
        };
        this.loading = false;
