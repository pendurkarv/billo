<template>
    <div>
      <master-data-table-toolbar title="Units" @search="setSearch" @create="openDialog('create')" @refresh="getItems" />
      <v-data-table
        :headers="headers"
        :items="items"
      ...
      <v-dialog v-model="dialog" max-width="300px" lazy persistent>
        <create-unit-form v-if="dialogComponent === 'create'" @create="postItemCreate" @cancel="close" />
        <edit-unit-form v-else-if="dialogComponent === 'edit'" :id="editedId" @update="postItemUpdate" @cancel="close" />
      </v-dialog>
...
import EditUnitForm from '@/components/unit/Edit';
...
export default {
  ...
  methods: {
    ...
    getItems() {
      ...
      return fetchMasterList('units', this.search, page, rowsPerPage, sortBy, descending ).then(response => {
    ...
    onItemDelete(item) {
      ...
      this.loading = true;            
      deleteMasterItem('units', id).then(() => {
        const snackbar = {
