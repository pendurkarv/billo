<template lang="html">
  <v-card>
    <v-card-title>
      <span class="headline">{{ title }} Transporter</span>
    </v-card-title>
    <v-card-text>
      <v-form ref="form" v-model="valid">
        ...
        ></v-text-field>
        <v-btn ref="submit" id="frm_transporter_submit" @click="add">{{ title }}</v-btn>
...
export default {
  ...
  props: ['title', 'transporter', 'formErrors', 'disabledFields', 'autofocus'],
  data() {
    return {
      valid: true,
      codeRules: [(code) => {
