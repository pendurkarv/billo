<template lang="html">
  <v-card>
    <v-card-title>
      <span class="headline">{{ title }} Category</span>
    </v-card-title>
    <v-card-text>
      <v-form ref="form" v-model="valid">
        ...
        ></v-text-field>
        <v-btn ref="submit" id="frm_category_submit" @click="add">{{ title }}</v-btn>
...
export default {
  name: 'category-form',
  props: ['title', 'category', 'formErrors', 'disabledFields', 'autofocus'],
  data() {
    return {
      valid: true,
      codeRules: [(code) => {
