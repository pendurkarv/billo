<template>
    <div>
      <master-data-table-toolbar title="Categories" @search="setSearch" @create="openDialog('create')" @refresh="getItems" />
      <v-data-table
        :headers="headers"
        :items="items"
      ...
      <v-dialog v-model="dialog" max-width="300px" lazy persistent>
        <create-category-form v-if="dialogComponent === 'create'" @create="postItemCreate" @cancel="close" />
        <edit-category-form v-else-if="dialogComponent === 'edit'" :id="editedId" @update="postItemUpdate" @cancel="close" />
      </v-dialog>
...
export default {
  ...
  methods: {
    ...
    getItems() {
      ...
      return fetchMasterList('categories', this.search, page, rowsPerPage, sortBy, descending ).then(response => {
    ...
    onItemDelete(item) {
      ...
      this.loading = true;            
      deleteMasterItem('categories', id).then(() => {
        const snackbar = {
          text: `Category "${id}" deleted`,
          show: true,
          color: 'success'
        };
        ...
        const snackbar = {
          text: `Category "${id}" could not be deleted`,
          show: true,
          color: 'error'
        };
        this.loading = false;
