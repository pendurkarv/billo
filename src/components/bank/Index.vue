<template>
    <div>
      <master-data-table-toolbar title="Banks" @search="setSearch" @create="openDialog('create')" @refresh="getItems" />
      <v-data-table
        :headers="headers"
        :items="items"
      ...
      <v-dialog v-model="dialog" max-width="300px" lazy persistent>
        <create-bank-form v-if="dialogComponent === 'create'" @create="postItemCreate" @cancel="close" />
        <edit-bank-form v-else-if="dialogComponent === 'edit'" :id="editedId" @update="postItemUpdate" @cancel="close" />
      </v-dialog>
...
import CreateBankForm from '@/components/bank/Create';
import EditBankForm from '@/components/bank/Edit';
...
export default {
  ...
  methods: {
    ...
    getItems() {
      ...
      return fetchMasterList('banks', this.search, page, rowsPerPage, sortBy, descending ).then(response => {
    ...
    onItemDelete(item) {
      ...
      this.loading = true;            
      deleteMasterItem('banks', id).then(() => {
        const snackbar = {
          text: `Bank "${id}" deleted`,
          show: true,
          color: 'success'
        };
        ...
        const snackbar = {
          text: `Bank "${id}" could not be deleted`,
          show: true,
          color: 'error'
        };
        this.loading = false;
